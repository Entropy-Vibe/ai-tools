...
{"id":"R-042","type":"response","milestone":"auth-system","summary":"Implemented refresh rotation in middleware, added 7-day expiry","tags":["auth","jwt","middleware"],"process_tag":"clean-execution","refs":["P-038"],"timestamp":"2026-02-25T10:32:00","content":"[key code and decisions]"}
```
...
- One entry for the response (R-XXX) with `process_tag` and optional `process_note`
- Write good summaries and honest process tags

### 4. Breadcrumb for tests (if applicable)
...
### 5. Flag for docs (if applicable)
- If this turn involved an architectural decision, non-obvious config, or pattern future devs need: add `"docs_worthy": true` to the HISTORY.jsonl entry
- If not, skip
